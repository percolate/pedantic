...
Request = namedtuple('Request', 'path method data query_data')
...
class RequestNotDefinedError(Exception):
    """
    Used to discover which APIs still require definitions. There are
    tasks to follow up with the eventual removal of this exemption.
    """
...
def parse_request(request):
    ...
    :param dict request: the request json
    ...
    :rtype nametuple: Contains relevant information from the request object.
        ...
        dict data: the data payload of the request
    ...
    for key in query_data:
        # everything from the query string is in a list, even if single element
...
def get_spec(request, schemas):
    ...
    :param namedtuple request: generated by `parse_request`
...
def _validate(data, schema):
    ...
    for error in errors:
        err_msg = "".join([err_msg, " - ", _get_pretty_path(error.absolute_path), ": ", error.message, "\n"])
...
def validate_request_against_schema(req_info, spec):
    ...
    if req_params and 'queryParameters' in spec:
        ...
        err_msg = " - `queryParameters` must be defined in the specification\n"
            ...
            if 'body' in value:
                ...
                try:
                    ...
                    msg = "".join([
                        ...
                        "\nCONTENT: ", unicode(json.loads(json.dumps(response.content))),
                        "\n"])
